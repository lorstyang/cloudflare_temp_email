...
const parseMail = async (
    ...
    metadata?: string | ExtractResult | null
) => {
    ...
    try {
        ...
        if (metadata) {
            try {
                let aiExtract: ExtractResult | null = null;
                if (typeof metadata === "string") {
                    const metaObj = JSON.parse(metadata);
                    aiExtract = metaObj?.ai_extract;
                } else if (typeof metadata === "object") {
                    if (metadata && 'ai_extract' in metadata) {
                        aiExtract = (metadata as any).ai_extract;
                    } else {
                        aiExtract = metadata as ExtractResult;
                    }
                }
...
export async function sendMailToTelegram(
    ...
    if (getBooleanValue(c.env.ENABLE_AI_EMAIL_EXTRACT)) {
        ...
        if (!isAuthCode && aiExtract?.type === "auth_code") {
            isAuthCode = true;
        }
        if (!isAuthCode) {
    ...
    const buildAndSend = async (targetUserId: string, msgs: LocaleMessages) => {
        const { mail } = await parseMail(
            msgs, parsedEmailContext, address, new Date().toUTCString(), metadata || aiExtract
        );
        if (!mail) return;
